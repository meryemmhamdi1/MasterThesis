...
def task_specific_attention(inputs, output_size,
    ...
    with tf.variable_scope('attention', reuse=tf.AUTO_REUSE) as scope:
        attention_context_vector = tf.Variable(tf.random_normal([output_size]), name='attention_context_vector',
                                               #shape=[output_size],
        ...
        vector_attn = tf.reduce_sum(tf.multiply(input_projection, attention_context_vector), axis=2, keep_dims=True) # keepdims=True
        attention_weights = tf.nn.softmax(vector_attn, dim=1)  # axis = 1
